...
class Sampler():
    ...
    def sample(self, data, mask_indices, candidates):
        ...
        # mask_indices = torch.nonzero(data.test_mask).squeeze().cpu().numpy()
        # print("Sampler: Indices")
        # print(indices_train)
        # choices = np.random.choice(indices_train,size=candidates,replace=False)
        choices = np.random.choice(mask_indices,size=candidates,replace=False)


        return (data.feature[choices],choices)
